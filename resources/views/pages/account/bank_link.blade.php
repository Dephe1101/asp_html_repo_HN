...
<x-app-layout>
    <x-slot name="head">
        <script>
            function submitForm() {
                const form = document.getElementById("form_bank_link");
                form.submit();
            }
        </script>
    </x-slot>
        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Liên kết ngân hàng</li>
                </ol>
            </nav>
        </div>
    </section>
    <section id="wrapper-content">
        <div class="container">
            <div class="box-credit-form py-5 d-flex flex-column">
                <form id="form_bank_link" method="POST" action="{{ route('bank_link.update') }}">
                    @csrf
                    @if (session('error'))
                    ...
                    <div class="box-upload-img">
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-around flex-ct-column w-100">
                                <div class="w-35 pt-2"><b class="fs-6">Số thẻ ATM</b> <span
                                        class="text-danger fw-bold">*</span></div>
                                <div class="w-65">
                                    <input name="bank_number" type="text" value="{{ $customer->bank_number }}" class="form-control @error('bank_number') is-invalid @enderror" required autofocus>
                                    @error('bank_number')
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-around flex-ct-column w-100">
                                <div class="w-35 pt-2"><b class="fs-6">Tên chủ tài khoản</b> <span
                                        class="text-danger fw-bold">*</span></div>
                                <div class="w-65">
                                    <input name="bank_account" value="{{ $customer->bank_account }}" type="text" class="form-control @error('bank_account') is-invalid @enderror" required autofocus>
                                    @error('bank_account')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-around flex-ct-column w-100">
                                <div class="w-35 pt-2"><b class="fs-6">Ngân hàng</b> <span
                                        class="text-danger fw-bold">*</span></div>
                                <div class="w-65">
                                    <select name="bank_id" id="select_box" class="form-select @error('bank_id') is-invalid @enderror" required autofocus>
                                        <option value="">Lựa chọn ngân hàng</option>
                                        @foreach ($banks as $key => $item)
                                            <option value="{{ $key }}" @if($customer->bank_id == $key) selected @endif>{{ $item }}</option>
                                        @endforeach
                                    </select>
                                    @error('bank_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-around flex-ct-column w-100">
                                <div class="w-35 pt-2"><b class="fs-6">Chọn loại thẻ của bạn</b> <span
                                        class="text-danger fw-bold">*</span></div>
                                <div class="w-65">
                                    <select name="bank_type" class="form-select @error('bank_type') is-invalid @enderror" required autofocus>
                                        <option value="">Lựa chọn loại thẻ</option>
                                        <option value="1" @if($customer->bank_type == '1') selected @endif>Thẻ visa</option>
                                        <option value="2" @if($customer->bank_type == '2') selected @endif>Thẻ ghi nợ</option>
                                        <option value="3" @if($customer->bank_type == '3') selected @endif>Khác</option>
                                    </select>
                                    @error('bank_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-around flex-ct-row w-100 text-center">
                                <div class="w-100">
                                    <p style="color: #da2028;"><b>Lưu ý: vui lòng nhập thông tin chính xác để được xét duyệt nhanh hơn</b></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </form>
                <a href="#" class="btn btn-primary mt-4" onclick="submitForm()">LIÊN KẾT THẺ</a>
            </div>
        </div>
    </section>
    <x-slot name="scripts">
        <script>
            ...
            function dselect(el, option = {}) {
                ...
                const searchInput = search ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="dselectSearch(event, this, '${classElement}', '${classToggler}', ${creatable})" type="text" class="form-control" placeholder="Nhập từ khóa" autofocus>` : ''
                ...
                function createDom() {
                    ...
                    <div class="dropdown ${classElement} ${additionalClass}">
                        <button class="text-start ${classToggler} ${!el.multiple && clearable ? classTogglerClearable : ''}" data-dselect-text="${!el.multiple && selectedText(el.options)}" type="button" data-bs-toggle="dropdown" aria-expanded="false"${autoclose}>
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <div class="${classNoResults} d-none">Không tìm thấy</div>
                        </div>
                        </div>
                        ${clearBtn}
                    </div>
